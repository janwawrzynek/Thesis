...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 16 23:35:43 2025

@author: janwawrzynek
"""

# attempting to put it all inside a loop
...
z_vals = np.linspace(z_init, z_final,200)


def coupled_boltzmann_eqs(z_vals, Y):
    T = np.linspace(T_final,T_init,200)
    ...
    entropy_density_value =entropy_density(z_vals)
    ...
    def gamma_aa_NN(T, m_N, m_a, mixing_parameter, f_a):
        ...
        # 3. Define lambda function: λ(a, b, c) = (a - b - c)² - 4bc
        def lambda_func(a, b, c):
            return (a - b - c)**2 - 4*b*c
        
        # 4. Define the reduced cross-section function
        def reduced_sigma(s):
            # Calculate the cross-section σ(s)
            sigma_val = cross_section_aa_NN(m_N, m_a, mixing_parameter, s, f_a)
            
            # Calculate λ(1, m_a²/s, m_a²/s) for two identical initial particles
            lam = lambda_func(1, m_a**2/s, m_a**2/s)
            
            # Return reduced cross-section: 2s σ(s) λ(1, m_a²/s, m_a²/s)
            return 2 * s * sigma_val * lam
        
        # 5. Define the integrand: s^{1/2} * reduced_sigma(s) * K₁(√s/T)
        def integrand(s):
        ...
        # 6. Perform the numerical integration
        ...
        # 7. Calculate gamma = T/(64π⁴) * integral
    ...
    prefactor = -1 / (z * H_value * entropy_density_value)
...
#below is some duplication to calculate initial Y_N values
#entropy density s
def entropy_density(z):
    entropy_density = ((2*(np.pi**2))/(45))*10.75*((m_N/z)**3)
    return entropy_density
...
#Y_N_eq = comoving_yield(n_N_eq,entropy_density)
# above didn't work
Y_N_initial = n_N_eq / entropy_density(z_init)

Y_a_initial = 1e-40
Y0 = [Y_N_initial, Y_a_initial]


sol = solve_ivp(fun=coupled_boltzmann_eqs, t_span=[z_init, z_final], y0=Y0, t_eval=z_vals, method='Radau')
